...
import { Download, Shield, Eye, Clock, CheckCircle } from 'lucide-react';
import * as XLSX from 'xlsx';

export default function AuditTrailView() {
  const { auditLogs, user } = useBotStore();
  ...
  const handleExportAuditExcel = () => {
    ...
    const reportMetadata = [
      ['FOREX BOT AI - REGULATORY COMPLIANCE AUDIT REPORT'],
      ['BAPPEBTI / GLOBAL REGULATORY COMPLIANCE COMPLIANT'],
      [],
      ['Report Generated By:', user.email],
      ['Report Date:', new Date().toLocaleString()],
      ['Total Logged Actions:', auditLogs.length.toString()],
      [],
            ...
            <tbody className="divide-y divide-slate-850/40 text-xs font-mono text-slate-350">
              {auditLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-955/20 transition-colors duration-250">
                  <td className="py-3.5 px-4 text-slate-200 font-semibold">{log.id}</td>
