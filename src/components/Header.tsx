...
export default function Header({ onMenuClick, onProfileClick }: HeaderProps) {
  ...
  return (
    <header className="h-20 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 lg:px-8 flex items-center justify-between z-20 gap-4">
      {/* Left side: Hamburger (mobile) & Financial Overview (scrollable/responsive) */}
      <div className="flex items-center gap-4 flex-1 min-w-0">
        {/* Hamburger Button */}
        <button
          onClick={onMenuClick}
          className="p-2.5 bg-slate-950/40 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 rounded-xl text-slate-400 hover:text-cyan-400 transition-all duration-300 lg:hidden shrink-0 cursor-pointer flex items-center justify-center"
      ...
      <div className="flex items-center gap-2.5 sm:gap-4 md:gap-5 shrink-0">
        ...
        {activeTrades.length > 0 && (
          <div className="hidden md:flex px-3 py-1.5 bg-cyan-950/50 border border-cyan-800/50 text-cyan-400 text-xs rounded-full font-mono items-center gap-1.5 animate-pulse animate-duration-1000">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2.5 md:px-4 md:py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-lg ${
            status === 'PANIC' 
        ...
        <button
          ...
          className="p-2.5 bg-slate-950/40 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 rounded-xl text-slate-400 hover:text-cyan-400 transition-all duration-300 active:scale-95 flex items-center justify-center cursor-pointer"
